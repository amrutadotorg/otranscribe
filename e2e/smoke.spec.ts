...
async function loadApp(page: Parameters<typeof test>[1] extends (arg: infer T) => unknown ? T : never) {
  await page.goto('/');
...
test.describe('Start screen', () => {
  ...
  test('shows all 4 media source buttons', async ({ page }) => {
    await loadApp(page);
    await expect(page.locator('#load-local-file-btn')).toBeVisible({ timeout: 10000 });
  ...
  test('YouTube button opens URL modal and validates', async ({ page }) => {
    ...
    await expect(page.locator('#url-modal-error')).toContainText('valid YouTube URL');
  ...
  test('Vimeo button opens URL modal and validates', async ({ page }) => {
    ...
    await page.locator('#url-modal-input').fill('https://vimeo.com/251865580/18d771e072');
    ...
    await expect(page.locator('#url-modal-input')).toHaveValue('https://vimeo.com/251865580/18d771e072');
